...
function redirect_pos_home(): void {
    header('Location: ' . app_url('/public/POS/index.php'));
    exit;
}

// Jika sudah login ke POS, langsung ke dashboard POS.
if (isset($_SESSION['pengguna_id'], $_SESSION['peran'])) {
    $role = (string)$_SESSION['peran'];
    if (in_array($role, ['kasir', 'manager', 'owner'], true)) {
        redirect_pos_home();
    }
}

// Auto-detect / refresh device session based on IP.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$deviceOk || $tokoIdFromDevice <= 0) {
    ...
    } elseif ($deviceType === 'gudang') {
        $error = 'Device tipe gudang tidak diizinkan login ke POS kasir.';
        ...
        if (!$user || !password_verify($password, (string)$user['password'])) {
            ...
            if (!in_array($role, ['kasir', 'manager', 'owner'], true)) {
                $error = 'Role akun tidak diizinkan untuk login POS.';
            } else {
...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        body { margin:0; min-height:100vh; display:grid; place-items:center; background:linear-gradient(160deg,#e2e8f0 0%,#f8fafc 60%,#dbeafe 100%); font-family:Inter,sans-serif; color:var(--text); }
        ...
        .meta { margin-top:12px; font-size:12px; color:var(--muted); background:#f8fafc; border:1px dashed var(--border); border-radius:10px; padding:10px; }
    </style>
</head>
<body>
    <div class="box">
        <h1>Login POS</h1>
        <p>Login khusus kasir/front POS. Login admin umum tetap di halaman login utama.</p>

        <?php if ($error): ?><div class="alert"><?= htmlspecialchars($error) ?></div><?php endif; ?>
        <?php if ($needDevice): ?><div class="alert">Sesi device belum valid. Daftarkan device dulu sebelum login POS.</div><?php endif; ?>
        ...
        <form method="post">
            ...
            <button type="submit">Masuk ke POS</button>
        </form>

        <div class="links">
            <a href="<?= htmlspecialchars(app_url('/public/device_register.php')) ?>">Registrasi Device</a>
            <a href="<?= htmlspecialchars(app_url('/public/admin/login.php')) ?>">Login Umum (Admin)</a>
        </div>

        <div class="meta">
            URL POS: <code><?= htmlspecialchars(app_url('/public/POS/')) ?></code>
        </div>
    </div>
</body>
</html>
