...
if (!$tokoId || !$produkId) die('Parameter tidak valid');
...
function col_exists(Database $db, string $table, string $column): bool {
...
$hasJasa = col_exists($pos_db, 'produk', 'is_jasa');
$hasKonsinyasi = col_exists($pos_db, 'produk', 'is_konsinyasi');
$exprJasa = $hasJasa ? 'COALESCE(p.is_jasa,0) AS is_jasa' : '0 AS is_jasa';
$exprKon = $hasKonsinyasi ? 'COALESCE(p.is_konsinyasi,0) AS is_konsinyasi' : '0 AS is_konsinyasi';

$sqlProduk = "SELECT p.produk_id,p.nama_produk,p.sku,p.barcode,p.kategori_id,p.supplier_id,p.satuan,p.harga_modal,p.min_stok,p.pajak_persen,p.aktif,$exprJasa,$exprKon
              FROM produk p WHERE p.produk_id=? AND p.toko_id=? AND p.deleted_at IS NULL LIMIT 1";
$stmt = $pos_db->prepare($sqlProduk);
...
$stmt->execute();
$hargaMap = [];
foreach (($stmt->get_result() ?: []) as $r) $hargaMap[$r['tipe']] = $r['harga_jual'];
$stmt->close();
...
$stmt = $pos_db->prepare("SELECT produk_id,sku,barcode FROM produk WHERE toko_id=? AND produk_id<>? AND deleted_at IS NULL");
$stmt->bind_param("ii", $tokoId, $produkId);
$stmt->execute();
    ...
    <div class="card">
        ...
        <form id="formEdit" enctype="multipart/form-data">
            ...
            <div class="grid">
                <div>
                    <label>SKU</label>
                    <input name="sku" required value="<?=htmlspecialchars((string)$produk['sku'])?>">
                    <div class="row-actions"><button type="button" id="btnSku">Auto SKU</button></div>
                    <small class="field-error" data-for="sku"></small>
                </div>
                <div>
                    <label>Barcode</label>
                    <input name="barcode" id="barcode" value="<?=htmlspecialchars((string)$produk['barcode'])?>">
                    <small class="field-error" data-for="barcode"></small>
                </div>
                <div class="full">
                    <label>Nama Produk</label>
                    <input name="nama_produk" required value="<?=htmlspecialchars((string)$produk['nama_produk'])?>">
                ...
                <div>
                    ...
                    <select name="supplier_id" required>
                        ...
                        <?php foreach($supplier as $s): ?>
                            <option value="<?=$s['supplier_id']?>" <?=$s['supplier_id']==$produk['supplier_id']?'selected':''?>><?=htmlspecialchars($s['nama_supplier'])?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    ...
                    <select name="kategori_id" required>
                        ...
                        <?php foreach($kategori as $k): ?>
                            <option value="<?=$k['kategori_id']?>" <?=$k['kategori_id']==$produk['kategori_id']?'selected':''?>><?=htmlspecialchars($k['nama_kategori'])?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    <label>Satuan</label>
                    <input name="satuan" required value="<?=htmlspecialchars((string)$produk['satuan'])?>">
                    <small class="field-error" data-for="satuan"></small>
                </div>
                <div>
                    <label>Harga Modal</label>
                    <input type="number" name="harga_modal" min="0" step="0.01" required value="<?=htmlspecialchars((string)$produk['harga_modal'])?>">
                    <small class="field-error" data-for="harga_modal"></small>
                </div>
                <div><label>Harga Ecer</label><input type="number" name="harga_ecer" min="0" step="0.01" required value="<?=htmlspecialchars((string)($hargaMap['ecer'] ?? 0))?>"></div>
                <div><label>Harga Grosir</label><input type="number" name="harga_grosir" min="0" step="0.01" required value="<?=htmlspecialchars((string)($hargaMap['grosir'] ?? 0))?>"></div>
                <div><label>Harga Reseller</label><input type="number" name="harga_reseller" min="0" step="0.01" required value="<?=htmlspecialchars((string)($hargaMap['reseller'] ?? 0))?>"></div>
                <div><label>Harga Member</label><input type="number" name="harga_member" min="0" step="0.01" required value="<?=htmlspecialchars((string)($hargaMap['member'] ?? 0))?>"></div>
                <div class="full">
                ...
                <div>
                    ...
                    <input type="number" name="min_stok" min="0" value="<?=htmlspecialchars((string)$produk['min_stok'])?>">
                    <small class="field-error" data-for="min_stok"></small>
                </div>
                <div>
                    <label>Pajak (%)</label>
                    <input type="number" name="pajak_persen" min="0" max="100" step="0.01" value="<?=htmlspecialchars((string)$produk['pajak_persen'])?>">
                </div>
                <div><label><input type="checkbox" name="is_jasa" id="is_jasa" value="1" <?=((int)$produk['is_jasa']===1?'checked':'')?>> Produk Jasa (tanpa stok)</label></div>
                <div><label><input type="checkbox" name="is_konsinyasi" value="1" <?=((int)$produk['is_konsinyasi']===1?'checked':'')?>> Produk Konsinyasi</label></div>
                <div class="full">
                    <label><input type="checkbox" name="isi_stok_awal" id="isi_stok_awal" value="1"> Tambah stok awal?</label>
                    <div id="stokAwal" class="collapse">
                        <div class="grid">
                            ...
                            <div><label>Harga Modal Stok Awal</label><input type="number" name="stok_awal_harga_modal" min="0" step="0.01"></div>
                        </div>
                    </div>
                </div>
                <div class="full">
                    <label>Foto (opsional, akan mengganti foto lama)</label>
                    <input type="file" name="foto" accept="image/*">
                    <div class="hint">MIME check aktif, nama file UUID, resize/compress, SVG/PHP berbahaya ditolak.</div>
                </div>
                <div class="full"><label><input type="checkbox" name="aktif" value="1" <?=((int)$produk['aktif']===1?'checked':'')?>> Aktif</label></div>
            </div>
